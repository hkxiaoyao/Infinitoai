...
- 你点击某一步后，会先执行当前步骤
- 如果当前步骤成功，后台会继续自动执行后续步骤直到结束
- 如果中途失败或你点击 `Stop`，流程会停在对应位置
...
6. 如果某一轮中途失败，Auto 会记录失败并自动开始下一轮

也就是说：

- 如果 Duck 邮箱可自动获取，整套流程更接近全自动
- 如果不能自动获取，Auto 会在邮箱阶段暂停
- 多轮模式下，单轮失败不会终止整个批次；只有手动停止或手动接管时才会中断后续轮次
...
- Step 4 优先匹配中文标题，例如 `你的 ChatGPT 代码为 040535`
- 不会再误收后续英文验证码标题
- 在 QQ 邮箱中，刷新时会优先点击 `重要联系人`，再点回 `收件箱`，避免仅点 `收件箱` 时列表不刷新
...
- Step 7 优先匹配英文标题，例如 `Your ChatGPT code is 281878`
- 不会再误收前面注册阶段的中文验证码标题
- 如果验证码提交后页面明确提示“验证码错误”，脚本会直接回邮箱刷新重查，并跳过刚刚那个错误验证码
- 这种重查不会再次先点 `重新发送电子邮件`
...
- 如果点击“继续”后页面仍进入“验证手机 / 输入手机号”流程，本轮会直接判失败
- 如果页面出现 `糟糕，出错了！` / `验证过程中出错 (max_check_attempts)` 这类致命错误页，本轮也会直接判失败
...
- 直接在 `Email` 输入框中粘贴邮箱
- 再执行 Step 3，后续步骤会自动续跑
- 如果仍想走 Auto 暂停恢复路径，也可以点 `Auto Continue`
